...
class EMD_WP_Session_Utils {
	...
	public static function create_dummy_session( $date = null ) {
		...
		if ( null === $date ) {
			 ...
			 * @param int
			 */
			$expires = time() + (int) apply_filters( 'emd_wp_session_expiration', 30 * 60 );
		}
		...
		// Store the session
        if (defined('EMD_WP_SESSION_USE_OPTIONS') && EMD_WP_SESSION_USE_OPTIONS) {
            add_option( "_emd_wp_session_{$session_id}", array(), '', 'no' );
            add_option( "_emd_wp_session_expires_{$session_id}", $expires, '', 'no' );
        } else {
	...
	public static function delete_old_sessions( $limit = 1000 ) {
		...
		$count = $wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->prefix}emd_sessions WHERE session_expiry < %s LIMIT %d",
				$now,
				$limit
			)
		);
	...
	public static function delete_all_sessions() {
        ...
        if (defined('EMD_WP_SESSION_USE_OPTIONS') && EMD_WP_SESSION_USE_OPTIONS) {
            ...
            $count = $wpdb->query( "TRUNCATE TABLE {$wpdb->prefix}emd_sessions" );
        }
	...
	public static function delete_all_sessions_from_options() {
        ...
        $count = $wpdb->query( "DELETE FROM $wpdb->options WHERE option_name LIKE '_emd_wp_session_%'" );
	...
	public static function get_session( $session_id, $default = array() ) {
		...
		$session = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}emd_sessions WHERE session_key = %s",
				esc_sql($session_id)
			),
			ARRAY_A
		);
	...
	public static function session_exists( $session_id ) {
        ...
        $exists = $wpdb->get_var($wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->prefix}emd_sessions WHERE session_key = %s", $session_id));
	...
	public static function add_session( $data = array() ) {
		...
		$result = $wpdb->insert( "{$wpdb->prefix}emd_sessions", $data );
	...
	public static function delete_session( $session_id = '' ) {
		...
		$wpdb->delete( "{$wpdb->prefix}emd_sessions", array( 'session_key' => $session_id ) );
	...
	public static function update_session( $session_id = '', $data = array() ) {
		...
		$result = $wpdb->update( "{$wpdb->prefix}emd_sessions", $data, array( 'session_key' => $session_id ) );
